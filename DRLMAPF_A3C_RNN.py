...
class Worker:
    def __init__(self, game, metaAgentID, workerID, a_size, groupLock):
        ...
        self.local_AC = ACNet(self.name, a_size, trainer,
                              True, GRID_SIZE, GLOBAL_NET_SCOPE)
    ...
    def parse_path(self, path):
        ...
        for t in range(len(path[:-1])):
            ...
            while len(move_queue) > 0:
                ...
                state, reward, done, nextActions, on_goal, blocking, valid_action = self.env._step(
                    (i+1, a))
    ...
    def work(self, max_episode_length, gamma, sess, coord, saver):
        ...
        with sess.as_default(), sess.graph.as_default():
            while self.shouldRun(coord, episode_count):
                ...
                swarm_reward[self.metaAgentID] = 0  # NOTE:先模仿
                if episode_count < PRIMING_LENGTH or demon_probs[self.metaAgentID] < DEMONSTRATION_PROB:
                    ...
                    self.synchronize()
                    if rollouts[self.metaAgentID] is not None:  # NOTE:
                        # assert not (rollouts[self.metaAgentID][self.agentID-1] == []), "rollout?"
                        i_l = self.train(
                        ...
                        if self.agentID == 1:
                            summary = tf.Summary()
                            summary.value.add(
                                tag='Losses/Imitation loss', simple_value=i_l)
                            global_summary.add_summary(
                                summary, int(episode_count))
                ...
                if OUTPUT_GIFS and self.workerID == 1 and ((not TRAINING) or (episode_count >= self.nextGIF)):
                    ...
                    episode_frames = [self.env._render(
                        mode='rgb_array', screen_height=900, screen_width=900)]
                ...
                while (not self.env.finished):  # Give me something!
                    ...
                    if saveGIF:
                        episode_frames.append(self.env._render(
                            mode='rgb_array', screen_width=900, screen_height=900))

                    episode_buffer.append([s[0], a, r, s1, d, v[0, 0], train_valid, pred_on_goal, int(
                        on_goal), pred_blocking, int(blocking), s[1], train_val])
                    ...
                    if TRAINING and (len(episode_buffer) % EXPERIENCE_BUFFER_SIZE == 0 or d):
                        ...
                        if d:
                            ...
                            s1Values[i_buf] = sess.run(self.local_AC.value,
                                                       feed_dict={self.local_AC.inputs: np.array([s[0]]), self.local_AC.goal_pos: [s[1]], self.local_AC.state_in[0]: rnn_state[0], self.local_AC.state_in[1]: rnn_state[1]})[0, 0]
                ...
                if not TRAINING:
                    ...
                    if self.workerID == 1:
                        episode_count += 1
                        print('({}) Thread {}: {} steps, {:.2f} reward ({} invalids).'.format(
                            episode_count, self.workerID, episode_step_count, episode_reward, episode_inv_count))
                    GIF_episode = int(episode_count)
                    mutex.release()
                else:
                    episode_count += 1./num_workers
                    ...
                    if episode_count % SUMMARY_WINDOW == 0:
                        ...
                        summary.value.add(
                            tag='Losses/Blocking Loss', simple_value=b_l)
                        summary.value.add(
                            tag='Losses/On Goal Loss', simple_value=og_l)
                        summary.value.add(
                ...
                if saveGIF:
                    ...
                    if TRAINING:
                        make_gif(images, '{}/episode_{:d}_{:d}_{:.1f}.gif'.format(gifs_path,
                                 GIF_episode, episode_step_count, swarm_reward[self.metaAgentID]))
                    else:
...
EXPERIENCE_BUFFER_SIZE = 16 #128
GRID_SIZE = 10  # the size of the FOV grid to apply to each agent
# the total size of the environment (length of one side)
ENVIRONMENT_SIZE = (10, 70)
OBSTACLE_DENSITY = (0, .5)  # range of densities
DIAG_MVMT = False  # Diagonal movements allowed?
a_size = 5 + int(DIAG_MVMT)*4
SUMMARY_WINDOW = 10
NUM_META_AGENTS = 1  # NOTE:不明白是什么 很奇怪，可能是每个thread上可计算的agent？
# int(multiprocessing.cpu_count() / (2 * NUM_META_AGENTS))NOTE:agents数量似乎与thread相关
NUM_THREADS = 1
NUM_BUFFERS = 1  # NO EXPERIENCE REPLAY int(NUM_THREADS / 2)
EPISODE_SAMPLES = EXPERIENCE_BUFFER_SIZE  # 64
LR_Q = 2.e-5  # 8.e-5 / NUM_THREADS # default: 1e-5
ADAPT_LR = True
# the coefficient A in LR_Q/sqrt(A*steps+1) for calculating LR
ADAPT_COEFF = 5.e-5
load_model = False
RESET_TRAINER = False
...
with tf.device("/cpu:0"):
    master_network = ACNet(GLOBAL_NET_SCOPE, a_size, None, False,
                           GRID_SIZE, GLOBAL_NET_SCOPE)  # Generate global network
...
if not TRAINING:
    print([np.mean(plan_durations), np.sqrt(np.var(plan_durations)), np.mean(
        np.asarray(plan_durations < max_episode_length, dtype=float))])
